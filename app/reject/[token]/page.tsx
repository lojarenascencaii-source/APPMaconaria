...
export default async function RejectPage({ params }: { params: Promise<{ token: string }> }) {
    ...
    try {
        // Find attendance by rejection token
        ...
        if (attendance.status === 'REJECTED') {
            return <SuccessState
                message="Esta presença já foi rejeitada anteriormente."
        ...
        if (attendance.status === 'APPROVED') {
            return <ErrorState message="Esta presença já foi aprovada e não pode ser rejeitada." />
        }

        // Check expiration
        if (attendance.tokenExpiry && new Date() > attendance.tokenExpiry) {
            return <ErrorState message="Este link de rejeição expirou." />
        }
        ...
        await prisma.attendance.update({
            ...
            data: {
                status: 'REJECTED',
                rejectionToken: null, // Invalidate token after use
                approvalToken: null, // Also invalidate approval token
                tokenExpiry: null,
            }
        })
        ...
        return <ErrorState message="Ocorreu um erro ao processar a rejeição." />
    }
}

function SuccessState({ message, apprentice, activity, date, isRejection }: {
    message: string,
    apprentice: string,
    activity: string,
    date: Date,
        ...
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
            <div className="bg-slate-900 p-8 rounded-xl border border-slate-800 shadow-2xl max-w-md w-full text-center">
                <div className="flex justify-center mb-6">
                    <div className={`w-16 h-16 ${isRejection ? 'bg-red-500/10' : 'bg-green-500/10'} rounded-full flex items-center justify-center`}>
                        {isRejection ? (
